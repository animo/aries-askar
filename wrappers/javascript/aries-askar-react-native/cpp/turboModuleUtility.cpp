...
jsi::Value createReturnValue(jsi::Runtime &rt, ErrorCode code,
  ...
  if (code == ErrorCode::Success) {
    auto valueWithoutNullptr = value == nullptr
                                   ? jsi::Value::null()
                                   : jsi::Value::createFromAscii(rt, value);
    object.setProperty(rt, "value", valueWithoutNullptr);
  }

  object.setProperty(rt, "errorCode", code);
...
jsi::Value createReturnValue(jsi::Runtime &rt, ErrorCode code, int32_t out) {
  ...
  if (code == ErrorCode::Success) {
    auto valueWithoutNullptr =
        value == nullptr ? jsi::Value::null() : jsi::Value(rt, value);
    object.setProperty(rt, "value", valueWithoutNullptr);
  }
...
template <>
jsi::Value createReturnValue(jsi::Runtime &rt, ErrorCode code, intrptr_t out) {
  auto object = jsi::Object(rt);

  if (code == ErrorCode::Success) {
    auto valueWithoutNullptr = value == nullptr ? jsi::Value::null() : jsi::Value(out));
    object.setProperty(rt, "value", valueWithoutNullPtr);
  }
...
jsi::Value createReturnValue(jsi::Runtime &rt, ErrorCode code,
                             LocalKeyHandle out) {
  return createReturnValue(rt, code, out == nullptr ? out : intptr_t(out._0));
}
...
jsi::Value createReturnValue(jsi::Runtime &rt, ErrorCode code,
  ...
  if (code == ErrorCode::Success) {
    if (out == nullptr || out.buffer == nullptr || out.tag_pos == nullptr ||
        out.nonce_pos == nullptr) {
      object.setProperty(rt, "value", jsi::Value::null());
    } else {
      auto object = jsi::Object(rt);
...
jsi::Value createReturnValue(jsi::Runtime &rt, ErrorCode code, AeadParams out) {
  ...
  if (code == ErrorCode::Success) {
    if (out == nullptr || out.nonce_length == nullptr ||
        out.nonce_length == nullptr) {
      object.setProperty(rt, "value", jsi::Value::null());
    } else {
      auto object = jsi::Object(rt);
...
void callbackWithResponse(CallbackId result, ErrorCode code,
  ...
  invoker->invokeAsync([result, code, response]() {
    ...
    auto out = createFromReturnValue(*rt, code, response);
